...
export default function OrderBrief(props) {
    const snacks = props.order.snacks.map((snack) => <h5><li key={snack.name}>{snack.name}   qty:{snack.qty}</li></h5>);
    const [modalVisible, setModalVisible] = useState(false);
    const handleClose = () => setModalVisible(false);
    const handleShow = () => setModalVisible(true);
    // A short message will be displayed when placing the mouse on the Edit outlined icon.
